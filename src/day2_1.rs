...
fn sum(games: &[Game]) -> u32 {
    games.iter().filter(|c| c.ok()).map(|c| c.id).sum()
}
...
";
    ...
    fn test_summing() {
        ...
        assert_eq!(sum(&input), 8);
    }
}

pub fn main() -> Result<()> {
    let content = "Game 1: 7 red, 14 blue; 2 blue, 3 red, 3 green; 4 green, 12 blue, 15 red; 3 green, 12 blue, 3 red; 11 red, 2 green
